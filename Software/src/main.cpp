...
static boolean ethernetON;   // ALL: Ethernet status

byte ecuNumber = 3; // NUMBER IS READ FROM D2 & D3: 0=PT, 1=CH, 2=BO, 3=GW
...
#include <QNEthernet.h>             // version 0.33.0-snapshot
...
void ethernet_setup()
{
    if (ecuNumber == 3) {
        ...
        DEBUG_PORT.print(F("Ethernet IP: "));
        DEBUG_PORT.println(Ethernet.localIP());

        server.begin();
        DEBUG_PORT.println(F("Ethernet server started"));
    }
} // ethernet_setup()

boolean isEthernetLinkUp()
{
...
void carlaSend(const CAN_message_t &msg)
{
    ...
    if (!activeClient || !activeClient.connected()) {
        ...
        if (gDebug & (ecuNumber == 3) && activeClient) {
            DEBUG_PORT.println(F("ETHERNET CLIENT CONNECTED (TX)"));
        }
    }

    if (activeClient && activeClient.connected()) {
        if (gDebug & (ecuNumber == 3)) {
            DEBUG_PORT.println(F("TX ETHERNET"));
        }
        ...
        serializeJson(doc, activeClient);
...
void carlaSniff()
{
    ...
    if (!activeClient || !activeClient.connected()) {
        ...
        if (gDebug & (ecuNumber == 3) && activeClient) {
            DEBUG_PORT.println(F("ETHERNET CLIENT CONNECTED (RX)"));
        }
    }

    if (activeClient && activeClient.connected()) {
        if (gDebug & (ecuNumber == 3)) {
            DEBUG_PORT.println(F("RX ETHERNET"));
        }
        ...
        while (activeClient.available()) {
            ...
            if (c == '\n') {
                ...
                if (jsonBuffer.length() > 0) {
                    ...
                    if (err) {
                        DEBUG_PORT.print(F("JSON deserialization failed: "));
                        DEBUG_PORT.println(err.c_str());
                        DEBUG_PORT.print(F("Failed JSON: "));
                        DEBUG_PORT.println(jsonBuffer);
                    } else {
                        out_msg.id = doc["id"] | 0;
                        out_msg.len = doc["dlc"] | 0;
                        JsonArray data = doc["data"].as<JsonArray>();

                        for (uint8_t i = 0; i < out_msg.len; i++) {
                            out_msg.buf[i] = data[i] | 0;
                        }

                        if (gDebug) {
                            DEBUG_PORT.print(F("MSG ID: 0x"));
                            DEBUG_PORT.print(out_msg.id, HEX);
                            DEBUG_PORT.print(F(" | MSG LEN: "));
                            DEBUG_PORT.print(out_msg.len);
                            DEBUG_PORT.print(F(" | MSG DATA: "));
                            for (int i = 0; i < out_msg.len; i++) {
                                DEBUG_PORT.print(out_msg.buf[i], HEX);
                                DEBUG_PORT.print(F(" "));
                            }
                            DEBUG_PORT.println();
                        }
                        ...
                        default:
                            if (gDebug) {
                                ...
                                DEBUG_PORT.println(out_msg.id, HEX);
                            }
                            break;
                        }
                    }
...
#else
// Functions not supported on Teensy 4.0
// Defined empty functions to avoid compilation errors
void ethernet_setup() {}
boolean isEthernetLinkUp() { return false; }
void carlaSend(const CAN_message_t &msg) {}
void carlaSniff() {}
#endif
...
void checkNext()
{
    ...
    if (ecuNumber == 0)
    {
        if ((next_resp[0] == 0x66) && (next_resp[1] == 0x0))
        {
            nextPage = 0;
            if (gDebug)
                DEBUG_PORT.println(F("CHANGE PAGE 0"));
        }
        else if ((next_resp[0] == 0x66) && (next_resp[1] == 0x1))
        {
            ...
            dispPowertrain();
            if (gDebug)
                DEBUG_PORT.println(F("CHANGE PAGE 1"));
        }
    }
    else if (ecuNumber == 1)
    {
        ...
        {
            nextPage = 1;
            dispChassis(ecu_data.engineRpmRAW, abs(ecu_data.speedKphRAW), ecu_data.engineCoolantTempRAW, ecu_data.fuelAmountRAW);
            if (gDebug)
                DEBUG_PORT.println(F("CHANGE PAGE 1"));
        }
        ...
        {
            nextPage = 2;
            dispSpeed(ecu_data.speedKphRAW);
            if (gDebug)
                DEBUG_PORT.println(F("CHANGE PAGE 2"));
        }
        else if ((next_resp[0] == 0x66) && (next_resp[1] == 0x3))
        {
            ...
            if (gDebug)
                DEBUG_PORT.println(F("CHANGE PAGE 3"));
        }
        else if ((next_resp[0] == 0x66) && (next_resp[1] == 0x4))
        {
            ...
            if (gDebug)
                DEBUG_PORT.println(F("CHANGE PAGE 4"));
        }
    }
    else if (ecuNumber == 2)
    {
...
void dispNext()
{
    ...
    else if (ecuNumber == 1)
    {
        if (nextPage == 1)
        {
            ...
            {
                DEBUG_PORT.println(F("ECU DATA CHANGED"));
                dispChassis(ecu_data.engineRpmRAW, abs(ecu_data.speedKphRAW), ecu_data.engineCoolantTempRAW, ecu_data.fuelAmountRAW);
            }
        }
        else if (nextPage == 2)
        {
            ...
            {
                dispSpeed(abs(ecu_data.speedKphRAW));
            }
        }
        else if (nextPage == 3)
        {
